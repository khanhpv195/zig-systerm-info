...
fn parseAny(comptime T: type, allocator: std.mem.Allocator, string: []const u8) !T {
    ...
    return switch (T) {
        ...
        *void => block: {
            ...
            const value = try parseInt(usize, string[2..], 16);
...
pub const Core = struct {
    ...
    pub const Iterator = struct {
        ...
        pub inline fn new(allocator: std.mem.Allocator) !@This() {
        ...
        pub fn next(this: *@This()) !?Core {
            ...
            while (true) {
                var line = reader.readUntilDelimiterAlloc(this.allocator, '\n', 4096) catch |err| {
                ...
                const div = std.mem.indexOfScalar(u8, line, ':') orelse return error.NoColon;

                const key_end = std.mem.lastIndexOfNone(u8, line[0..div], " \t") orelse return error.NoSpaces;

                const key = line[0 .. key_end + 1];
                const value = line[div + 2 ..];

                inline for (std.meta.fields(Core.Inner)) |field| {
                    if (std.mem.eql(u8, key, field.name)) {
                        @field(core, field.name) = try parseAny(@TypeOf(@field(core, field.name).?), this.allocator, value);
                        break;
                    }
                }
            }
    ...
    pub fn deinit(this: *@This()) void {
        inline for (std.meta.fields(Core.Inner)) |field| {
            if (@field(this.inner, field.name)) |item| {
